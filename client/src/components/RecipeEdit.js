...
import {
  setUnits, setRecipe, setRecipeName, setRecipeSource, setRecipeStepDescription,
  addRecipeStep, deleteRecipeStep, setRecipePortions, setRecipeAdditionAmount,
  setRecipeAdditionUnit, setIngredients, addRecipeAddition, deleteRecipeAddition, setRecipeAdditionIngredient
} from '../actions'
...
class RecipeEdit extends Component {
  ...
  handleSave = (e) => {
    ...
    }).then((e) => e.json()).then((body) => body.data.createIngredient))).then((newIngredients) => {
          ...
          query: `
            mutation {
              updateRecipe(
                ...
                additions: [
                  ${this.props.recipe.additions.map((add) => {
          ...
          <table>
            ...
            <tbody>
              {this.props.recipe.additions.map((addition, idx) => <tr key={idx}>
                ...
                <td><button onClick={(e) => {
                  e.preventDefault();
                  this.props.deleteRecipeAddition(idx);
                }}>Delete</button></td>
              </tr>)}
            </tbody>
          </table>
